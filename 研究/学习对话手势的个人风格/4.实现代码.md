...
```
python -m audio_to_multiple_pose_gan.train --gans 1 --name test_run --arch_g audio_to_pose_gans --arch_d D_patchgan --speaker chemistry --output_path ../tmp --train_csv ../train.csv --epochs 3
```
...
优化器|详细
-|-
self.train_D|tf.train.AdamOptimizer( learning_rate=self.args.lr_d) .minimize(loss=self.D_loss, var_list=tf.trainable_variables( scope=D_SCOPE))
self.train_G|tf.train.AdamOptimizer( learning_rate=self.args.lr_g) .minimize( loss=self.G_loss, var_list=trainable_variables)

loss|详细
-|-
self.G_loss|self.G_loss = self.pose_regloss + args.lambda_gan * self.G_gan_loss
self.pose_regloss|
self.G_gan_loss|
